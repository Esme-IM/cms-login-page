 ...
 * -------------------------------------------------------------------------- */
if ( ! function_exists( 'innermedia_login_bootstrap_updater' ) ) {
	function innermedia_login_bootstrap_updater() {
		static $checker = null;
		if ( null !== $checker ) {
			return $checker;
		}
...
add_action( 'plugins_loaded', 'innermedia_login_bootstrap_updater' );

/* -----------------------------------------------------------------------------
 * Login page styles + scripts
 * -------------------------------------------------------------------------- */
if ( ! function_exists( 'innermedia_login_print_styles' ) ) {
	function innermedia_login_print_styles() {
		echo "<style id=\"innermedia-login-css\">\n" . innermedia_login_css() . "\n</style>\n";
	}
}
add_action( 'login_head', 'innermedia_login_print_styles', 20 );

if ( ! function_exists( 'innermedia_login_print_scripts' ) ) {
	function innermedia_login_print_scripts() {
		echo "<script id=\"innermedia-login-js\">\n" . innermedia_login_js() . "\n</script>\n";
	}
}
add_action( 'login_footer', 'innermedia_login_print_scripts' );

if ( ! function_exists( 'innermedia_login_css' ) ) {
	function innermedia_login_css() {
...
if ( ! function_exists( 'innermedia_login_js' ) ) {
	function innermedia_login_js() {
		...
		$user_json = wp_json_encode( $svg_user );
		$lock_json = wp_json_encode( $svg_lock );

		return "(function(){
			var svgUser = {$user_json};
			var svgLock = {$lock_json};
...
/* -----------------------------------------------------------------------------
 * Login page markup tweaks
 * -------------------------------------------------------------------------- */
add_filter( 'login_display_language_dropdown', '__return_false' );

if ( ! function_exists( 'innermedia_login_welcome' ) ) {
	function innermedia_login_welcome() {
		echo '<div class="im-welcome"><h2>'
			. esc_html__( 'Welcome Back', 'innermedia-cms-login-page' )
			. '</h2><p>'
			. esc_html__( 'Sign in to manage your CMS.', 'innermedia-cms-login-page' )
			. '</p></div>';
	}
}
add_action( 'login_form_top', 'innermedia_login_welcome' );

if ( ! function_exists( 'innermedia_login_button_label' ) ) {
...
// Change URL/text for the login screen logo.
...
if ( ! function_exists( 'innermedia_login_footer_message' ) ) {
	function innermedia_login_footer_message() {
		$policy_link = get_privacy_policy_url();
		$year        = wp_date( 'Y' );

		echo '<div class="im-footer-links">';
		echo '<a class="support" href="' . esc_url( 'mailto:support@innermedia.co.uk' ) . '">'
			. esc_html__( 'Contact support', 'innermedia-cms-login-page' )
			. '</a>';
		if ( $policy_link ) {
			echo '<a class="privacy" href="' . esc_url( $policy_link ) . '" target="_blank" rel="noopener noreferrer">'
				. esc_html__( 'Privacy Policy', 'innermedia-cms-login-page' )
				. '</a>';
		}
		echo '</div>';
